...
public class Saml2WebSsoAuthenticationRequestFilter extends OncePerRequestFilter {
	...
	private void sendPost(HttpServletRequest request, HttpServletResponse response,
		...
		UriComponentsBuilder uriBuilder = UriComponentsBuilder
			.fromUriString(authenticationRequest.getAuthenticationRequestUri());
		addPostParameter(Saml2ParameterNames.SAML_REQUEST, authenticationRequest.getSamlRequest(), uriBuilder);
		addPostParameter(Saml2ParameterNames.RELAY_STATE, authenticationRequest.getRelayState(), uriBuilder);
		this.formPostRedirectStrategy.sendRedirect(request, response, uriBuilder.build().toUriString());
	}

	private void addPostParameter(String name, String value, UriComponentsBuilder builder) {
		Assert.hasText(name, "name cannot be empty or null");
		if (StringUtils.hasText(value)) {
