...
`spring-security-6.5` is a Git repository at `spring-security-6.5`. This change releases `6.5.11-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.
...
- Publish `6.5.11-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
- Verify approved build/test evidence or exception, publication metadata, and consumer behavior.
- Prove target absence before the single authorized Nexus deployment.
- Verify immutable remote assets before creating annotated tag `v6.5.11-nes.patch.1`.
...
Only the manifest owner for `spring-security-6.5` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `6.5.11-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.

Any internal version ending in `-SNAPSHOT` is a hard failure.

### 3. Bind validation and publication to one release commit

Reuse approved development build/test evidence when no intervening source or
test change invalidates it. An explicit operator exception may authorize not
rerunning these expensive commands. Generate local publications incrementally
without `clean` or tests:

- Local publication/effective POM: `JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF-8 ./gradlew publishToMavenLocal -x test -PbuildSrc.skipTests=true --max-workers=3`
...
- Incremental Nexus RELEASE deploy using `JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF-8 ./gradlew publishAllPublicationsToNexusRepository -x test -PbuildSrc.skipTests=true --max-workers=3`;
- release commit push;
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v6.5.11-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v6.5.11-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
