...
== NES Development Line

This branch currently develops `5.8.16-nes.patch.2-SNAPSHOT`. The representative
coordinate is `cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-core:5.8.16-nes.patch.2-SNAPSHOT`.
The previous immutable release is `5.8.16-nes.patch.1`, tagged as
`v5.8.16-nes.patch.1`.
It depends on the verified Spring Framework RELEASE
`cn.bjca.footstone.bpring:bjca-footstone-bpring-framework-bom:5.3.39-nes.patch.1`.

Release validation uses `make build`, the approved relevant Gradle tests, and
`make install` followed by generated POM scanning and a local consumer smoke test.
`make verify-published-security` validates the current snapshot in an isolated local
repository. A formal patch.2 release requires a separate change that removes
`-SNAPSHOT`, verifies Nexus RELEASE absence, deploys once, and creates
`v5.8.16-nes.patch.2` only after remote verification.
