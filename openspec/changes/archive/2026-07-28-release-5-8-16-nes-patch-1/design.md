...
`spring-security-5.8` is a Git repository at `spring-security-5.8`. This change releases `5.8.16-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.

Representative publications:

`cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-core:5.8.16-nes.patch.1`
...
- Publish `5.8.16-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v5.8.16-nes.patch.1`.
...
Only the manifest owner for `spring-security-5.8` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `5.8.16-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
- Build: `make build`
- Tests: `./gradlew test -x integrationTest -PbuildSrc.skipTests=true`
- Local publication/effective POM: `make install`

Run representative RELEASE-only consumer validation after local publication. Update `README.adoc`, `doc/GAV_MAPPING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.

The successful `make build` at `74d3404b947200fb0bb856342ea47f67af78dde0`
and the later targeted crypto, SAML2, and OAuth2 JOSE tests are reusable because
that commit is an ancestor of the preparation HEAD and the intervening diff has
no production or test source changes. This does not waive the release-version
`make install`, generated POM scan, or local consumer smoke test.
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v5.8.16-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v5.8.16-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
