...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-security-5.8` in `spring-security-5.8`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `5.8.16-nes.patch.1` without exposing credentials.
...
- [x] 2.1 Set the component version to `5.8.16-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-core:5.8.16-nes.patch.1`.
...
- [x] 2.5 Update `README.adoc`, `doc/GAV_MAPPING.md` with version, GAVs, required RELEASE dependencies, validation commands, Nexus location, exclusions, and notable constraints.
- [x] 2.6 Review the full working diff and confirm it contains no unrelated source changes or credentials.

## 3. Local verification

- [x] 3.1 Reuse the successful `make build` evidence at ancestor commit `74d3404b947200fb0bb856342ea47f67af78dde0` after reviewing the intervening diff.
- [x] 3.2 Reuse the successful targeted crypto, SAML2, and OAuth2 JOSE test evidence after confirming no intervening production or test source changes.
- [x] 3.3 Generate local publications/effective POMs: `make install`.
...
- [x] 4.3 Create the dedicated release commit for `spring-security-5.8` `5.8.16-nes.patch.1`.
...
- [x] 7.1 Confirm `v5.8.16-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [x] 7.2 Create annotated tag `v5.8.16-nes.patch.1` on the exact release commit only after `nexus-verified`.
